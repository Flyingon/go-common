...
// CloseRedisClt 关闭redis连接池
func CloseRedisClt(key string) error {
	cltPtr, exist := redisCltMap.Load(key)
	if !exist {
		return nil
	}
...
// GetRedisCltKeys 查询所有注册的redis
func GetRedisCltKeys() []string {
	var ret []string
	redisCltMap.Range(func(kPtr, vPtr interface{}) bool {
		if k, ok := kPtr.(string); ok {
			ret = append(ret, k)
		}
		return true
	})
...
// Init redis clt初始化
func Init(confList map[string]*TRedisConf) {
